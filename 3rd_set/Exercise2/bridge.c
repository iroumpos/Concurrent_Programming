...
void entry_red(void* args) {
    ...
    while (1) {
		if(arguments->blue_in > 0 || arguments->red_in == arguments->capacity || arguments->bluew>0){    
       		arguments->redw++;
...
void entry_blue(void*args) {
    ...
    pthread_mutex_lock(&arguments->monitor);
	while (1) {
    	if ((arguments->red_in > 0) || (arguments->blue_in == arguments->capacity) || arguments->redw>0) {
        	arguments->bluew++;
